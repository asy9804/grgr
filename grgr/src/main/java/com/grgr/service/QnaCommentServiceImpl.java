...
public class QnaCommentServiceImpl implements QnaCommentService {
	...
	@Override
	public void getQnaCommentCount(int qnaBno) throws NoCommentsException {
		if(qnaCommentDAO.qnaCommentCount(qnaBno) < 1) {
			throw new NoCommentsException();
		}
	}

	@Override
	public Map<String, Object> getQnaCommentList(int qnaBno, int pageNum) throws NoCommentsException {
		int totalComment = qnaCommentDAO.qnaCommentCount(qnaBno);
		
		if (totalComment == 0) {
			throw new NoCommentsException("해당 게시글에는 댓글이 없습니다.");
		}
